 ...
 */
function portfolio_lite_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size', 'portfolio-lite' ),
...
add_action( 'customize_register', 'portfolio_lite_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function portfolio_lite_logo_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = portfolio_lite_logo_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = portfolio_lite_logo_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'portfolio_lite_logo_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function portfolio_lite_logo_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
 ...
 */
function portfolio_lite_logo_customize_preview_js() {
	wp_enqueue_script( 'portfolio-lite-logo-customizer', get_template_directory_uri() . '/customizer/js/logo/customize-preview.js', array( 'jquery', 'customize-preview' ), '201709081119', true );
}
add_action( 'customize_preview_init', 'portfolio_lite_logo_customize_preview_js' );

/**
 * JS handlers for Customizer Controls
 */
function portfolio_lite_logo_customize_controls_js() {
		wp_enqueue_script( 'portfolio-lite-logo-customizer-controls', get_template_directory_uri() . '/customizer/js/logo/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'portfolio_lite_logo_customize_controls_js' );

/**
 * Adds CSS to the Customizer controls.
 */
function portfolio_lite_logo_customize_css() {
